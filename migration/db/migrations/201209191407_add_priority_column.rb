...
class AddPriorityColumn < ActiveRecord::Migration
  ...
  def self.up
    ...
    column_mapping.label = 'label.priority'
    column_mapping.path_expression = 'priority.name'
    column_mapping.sortable = true
    column_mapping.default_order = 10122
    column_mapping.column_mapping_group = grp_event_event_details
    column_mapping.name = 'event_search_priority'
  ...
  def self.down
    column = ColumnMapping.find(:first, :conditions => {:name => 'event_search_priority'})
