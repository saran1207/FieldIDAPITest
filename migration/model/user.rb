...
class User < ActiveRecord::Base
  ...
  def self.findByDisplayName(tenant, displayName)
    ...
    user = User.find(:first, :conditions => ["r_tenant = :tenantId and (lower(trim(userid)) = :name or lower(trim(lastname)) || ',' || lower(trim(firstname)) = :name or lower(trim(firstname)) || lower(trim(lastname)) = :name)", {:tenantId => tenant.id, :name => findName}])
