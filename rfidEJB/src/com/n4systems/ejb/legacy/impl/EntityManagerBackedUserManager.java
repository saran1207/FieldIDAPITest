...
public class EntityManagerBackedUserManager implements UserManager {
	...
	public User findUser(String tenantName, String userID, String plainTextPassword) {
		...
		try {
			...
			query.setParameter ("password", User.hashPassword(plainTextPassword));
	...
	public User findUser(String tenantName, String rfidNumber) {
		...
		List<User> userBeans = (List<User>)em.createQuery(generateFromClause() + "where u.tenant.name = :tenantName and u.hashSecurityCardNumber = :hashSecurityCardNumber and u.deleted = false and u.active = true")
			.setParameter ("hashSecurityCardNumber", User.hashSecurityCardNumber(rfidNumber))
	...
	public boolean userIdIsUnique(Long tenantId, String userId, Long currentUserId) {
		...
		if(currentUserId != null) {
			currentUserFilter = " and u.id <> :id";
		}
		
		if (userId != null) userId = userId.toLowerCase();
		
		Query query = em.createQuery("select count(u) from " + User.class.getName() + " u where u.tenant.id = :tenantId and lower(u.userID) = :userId" + currentUserFilter);
	...
	public boolean userRfidIsUnique(Long tenantId, String userRfid, Long currentUserId) {
		...
		Query query = em.createQuery("select count(u) from " + User.class.getName() + " u where u.tenant.id = :tenantId and u.hashSecurityCardNumber = :userRfid" + currentUserFilter);
		
		query.setParameter ("tenantId", tenantId);
		query.setParameter ("userRfid", User.hashSecurityCardNumber(userRfid));
	...
	public User findUserBeanByID(String tenantName, String userID) {
		...
		try {
			userBean = (User)em.createQuery(generateFromClause() + "where u.tenant.name = :tenantName and u.userID = :userID and u.deleted = false and u.active = true ")
	...
	public User findUser(Long id, Long tenantId) {
		Query query = em.createQuery("from " + User.class.getName() + " u where u.tenant.id = :tenantId and u.id = :id");
	...
	public Pager<User> getUsers( SecurityFilter filter, boolean onlyActive, int pageNumber, int pageSize, String nameFilter, UserType userType, CustomerOrg customer ) {
		...
		String queryString = "from " + User.class.getName() + " ub where  " + filter.produceWhereClause(User.class, "ub") 
	...
	public List<ListingPair> getUserList( SecurityFilter filter ) {
		...
		String queryString = "select new com.n4systems.util.ListingPair( ub.id, CONCAT(ub.firstName, ' ', ub.lastName ) ) from " + User.class.getName() + " ub where ub.system = false and ub.active = true and " + 
			filter.produceWhereClause(User.class, "ub") + " ORDER BY ub.firstName, ub.lastName";
	...
	public List<ListingPair> getExaminers( SecurityFilter filter ) {
		...
		String queryString = "select DISTINCT ub from " + User.class.getName() + " ub where ub.active = true and deleted = false and ub.system = false and ( " + 
					...
					justTenantFilter.produceWhereClause(User.class, "ub") + " AND ub.owner.customerOrg IS NULL) )" +
	...
	private String generateFromClause() {
		return "from " + User.class.getName() + " " + defaultTablePrefix + " ";
	}
