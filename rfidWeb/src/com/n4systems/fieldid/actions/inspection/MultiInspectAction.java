...
public class MultiInspectAction extends AbstractAction {
	...
	public String doInspectionTypes() {
		...
		eventTypes = loaderBackedCommonInspectionTypeHandler
				.findCommonInspectionTypesFor(assetIds);

		return SUCCESS;
	}

	private CommonInspectionTypeHandler createCommonInspectionTypeHandler() {
		return new LoaderBackedCommonInspectionTypeHandler(
				new CommonProductTypeDatabaseLoader(getSecurityFilter(),
						ConfigContext.getCurrentContext()));
	}

	public String doPerformEvent() {
		assets = persistenceManager.findAll(new QueryBuilder<Product>(
				Product.class, getSecurityFilter()).addWhere(Comparator.IN,
	...
	public void setEventTypeId(Long type) {
		if (type == null) {
			inspectionType = null;
				...
				|| !type.equals(inspectionType.getId())) {
			inspectionType = persistenceManager.find(InspectionType.class,
					type, getTenantId(), "sections", "supportedProofTests", "infoFieldNames");
		}
	}
