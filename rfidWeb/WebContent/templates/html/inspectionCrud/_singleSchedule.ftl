<div class="infoSet <#if inspectionDate?exists>autoSuggested</#if>" id="schedule_${index}">
	...
	<span  class="fieldHolder">
		<@s.hidden name="nextSchedule[${index}].type" id="nextInspectionType_${index}" value="${type.id}"/>
		<@s.hidden name="nextSchedule[${index}].date" id="nextInspectionDate_${index}" value="${date}"/>
		<#if securityGuard.projectsEnabled && job?exists>
			<@s.hidden name="nextSchedule[${index}].job" id="nextInspectionJob_${index}" value="${job.id}"/>
		</#if>
