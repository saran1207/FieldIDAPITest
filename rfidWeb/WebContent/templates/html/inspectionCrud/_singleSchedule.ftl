<div class="infoSet <#if inspectionDate?exists>autoSuggested</#if>" id="schedule_${index}">
	...
	<span  class="fieldHolder">
		<@s.hidden name="nextSchedules[${index}].type" id="nextInspectionType_${index}" value="${type.id}"/>
		<@s.hidden name="nextSchedules[${index}].date" id="nextInspectionDate_${index}" value="${date}"/>
		<#if securityGuard.projectsEnabled && job?exists>
			<@s.hidden name="nextSchedules[${index}].job" id="nextInspectionJob_${index}" value="${job.id}"/>
		</#if>
