...
public class InspectionBuilder extends BaseBuilder<Inspection> {
	...
	public static InspectionBuilder anInspection() {
		return new InspectionBuilder(anInspectionType().build(), anAsset().build(), new ArrayList<SubInspection>(), new Date(), new ArrayList<FileAttachment>(), true, null, aPrimaryOrg().build(), aUser().build(), aTenant().build());
	}
	
	private InspectionBuilder(InspectionType type, Asset asset, List<SubInspection> subInspections, Date datePerformed, List<FileAttachment> attachements, boolean printable, AssignedToUpdate assignedTo, BaseOrg owner, User performedBy, Tenant tenant) {
	...
	public InspectionBuilder ofType(InspectionType type) {
		return new InspectionBuilder(type, asset, subInspections, datePerformed, attachments, printable, assignedTo, owner, performedBy, tenant);
	}
	
	public InspectionBuilder on(Asset asset) {
		return new InspectionBuilder(inspectionType, asset, subInspections, datePerformed, attachments, printable, assignedTo, owner, performedBy, tenant);
	}
	...
	public InspectionBuilder withNoAssignedToUpdate() {
		return new InspectionBuilder(inspectionType, asset, subInspections, datePerformed, attachments, printable, null, owner, performedBy, tenant);
	}

	public InspectionBuilder withAssignedToUpdate(AssignedToUpdate assignToUpdate) {
		return new InspectionBuilder(inspectionType, asset, subInspections, datePerformed, attachments, printable, assignToUpdate, owner, performedBy, tenant);
	}
