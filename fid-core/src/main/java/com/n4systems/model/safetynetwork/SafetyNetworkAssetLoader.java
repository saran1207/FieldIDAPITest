...
public class SafetyNetworkAssetLoader extends SecurityFilteredLoader<Asset> {
	...
	public SafetyNetworkAssetLoader(SecurityFilter filter, FilteredIdLoader<Asset> assetLoader, VendorLinkedOrgLoader vendorOrgLoader) {
		super(filter);
		this.assetLoader = assetLoader;
		this.vendorOrgLoader = vendorOrgLoader;
	}
	
	public SafetyNetworkAssetLoader(SecurityFilter filter) {
	...
	protected Asset load(EntityManager em, SecurityFilter filter) {
		...
		// security check.
		Asset asset = PostFetcher.postFetchFields(assetLoader.load(em, new OpenSecurityFilter()), postLoadFields);
	...
	public SafetyNetworkAssetLoader setAssetId(Long assetId) {
		assetLoader.setId(assetId);
		return this;
	}
