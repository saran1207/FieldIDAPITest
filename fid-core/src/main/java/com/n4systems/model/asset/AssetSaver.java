...
public class AssetSaver extends Saver<Asset> {
	...
	public Asset update(EntityManager em, Asset asset) {
		...
		boolean linkedAssetChanged = asset.linkedAssetHasChanged();

		setModifiedByOnAsset(asset);
		Asset managedAsset = super.update(em, asset);
		
		managedAsset = resave(em, managedAsset);
		
		if (linkedAssetChanged && !asset.isNew()) {
	...
	private void forceNetworkIdRecalc(EntityManager em, Asset asset) {
		List<Asset> linkedAssets = linkedAssetLoader.setAsset(asset).load(em);
	...
	private void setModifiedByOnAsset(Asset asset) {
		if (modifiedBy != null) {
