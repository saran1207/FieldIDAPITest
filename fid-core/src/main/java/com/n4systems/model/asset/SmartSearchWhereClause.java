...
public class SmartSearchWhereClause extends WhereParameterGroup {
	...
	private void init() {
		if (useIdentifier) {
			addClause(WhereClauseFactory.create("identifier", searchText, WhereParameter.TRIM, ChainOp.OR));
		}
		
		if (useRfidNumber) {
			addClause(WhereClauseFactory.create("rfidNumber", searchText, WhereParameter.TRIM, ChainOp.OR));
		}
		
		if (useRefNumber) {
			addClause(WhereClauseFactory.create("customerRefNumber", searchText, WhereParameter.TRIM, ChainOp.OR));
		}
	}
