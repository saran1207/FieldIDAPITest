...
public class EventExporter implements Exporter {
	...
	private final ListLoader<ThingEvent> eventLoader;
	private final ModelToViewConverter<ThingEvent, EventView> converter;

	public EventExporter(ListLoader<ThingEvent> eventLoader, ExportMapMarshaller<EventView> marshaller, ModelToViewConverter<ThingEvent, EventView> converter) {
		this.eventLoader = eventLoader;
		this.marshaller = marshaller;
		this.converter = converter;
	}
	
	public EventExporter(ListLoader<ThingEvent> eventLoader, NextEventDateByEventLoader nextDateLoader) {
		this(eventLoader, new ExportMapMarshaller<EventView>(EventView.class), new EventToViewConverter(nextDateLoader));
	}

	public EventExporter(ListLoader<ThingEvent> eventLoader, NextEventDateByEventLoader nextDateLoader, ExportMapMarshaller<EventView> exportMapMarshaler) {
	...
	public void export(MapWriter mapWriter) throws ExportException {
		...
		for (ThingEvent event : eventLoader.load()) {
			try {
