...
JasperPrint jasperPrint = JasperFillManager.fillReport(jasperReport, reportMap, new JRBeanCollectionDataSource(reportCollection));
	...
	private void addProofTestParams(Map<String, Object> reportMap, ThingEvent event) {
        ...
        if (itr.hasNext()) {
            ...
            if(s3Service.assetProofTestExists(proofTest)){
                URL chartUrl = s3Service.getAssetProofTestUrl(event.getAsset().getMobileGUID(), event.getMobileGUID(), proofTest.getProofTestInfo().getProofTestFileName());
	...
	private void addNextEventScheduleParams(Map<String, Object> reportMap, AbstractEvent<?,Asset> event, DateTimeDefiner dateDefiner) {
		Event openEvent = eventScheduleService.getNextEventSchedule(event.getTarget().getId(), event.getType().getId());
		if (openEvent != null) {
