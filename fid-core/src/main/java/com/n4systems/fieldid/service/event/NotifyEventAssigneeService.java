...
public class NotifyEventAssigneeService extends FieldIdPersistenceService {
    ...
    private static final String DOWNLOAD_ICAL_INVITATION_FRAGMENT = "/fieldid/file/downloadCalendarInvitation.action?uniqueID={0}";

    private static final String GOOGLE_CALENDAR_PATH_FRAGMENT = "http://www.google.com/calendar/render?action=TEMPLATE&text={0}&dates={1}";
    ...
    private TemplateMailMessage createMailMessage(List<Event> events, User assignee) {
        ...
        Map<Long, String> iCalInvitationUrlMap = createICalInvitationUrlMap(events);
        Map<Long, String> googleCalendarUrlMap = createGoogleCalendarUrlMap(events);
        ...
        msg.getTemplateMap().put("iCalInvitationUrlMap", iCalInvitationUrlMap);
        msg.getTemplateMap().put("googleCalendarUrlMap", googleCalendarUrlMap);
    ...
    private Map<Long, String> createICalInvitationUrlMap(List<Event> events) {
        return events.stream()
                .collect(Collectors.toMap(Event::getId, this::createICalInvitationUrl));
    }

    private String createICalInvitationUrl(Event event) {
        String iCalInvitationUrl = SystemUrlUtil.getSystemUrl(event.getTenant()) + DOWNLOAD_ICAL_INVITATION_FRAGMENT;
        return MessageFormat.format(iCalInvitationUrl, event.getId().toString());
    }

    private Map<Long, String> createGoogleCalendarUrlMap(List<Event> events) {
        return events.stream()
                .collect(Collectors.toMap(Event::getId, this::createGoogleCalendarUrl));
    }

    private String createGoogleCalendarUrl(Event event) {
        try {
            return MessageFormat.format(GOOGLE_CALENDAR_PATH_FRAGMENT, URLEncoder.encode(getCalendarInvitationSubject(event), "utf-8"), getGoogleCalendarDates(event));
        } catch (UnsupportedEncodingException e) {
            logger.error("Failed to generate google calendar link for event: " + event.getId(), e);
            return "";
        }
    }

    public static String getCalendarInvitationSubject(Event event) {
        String subject="";
        if (event != null && event.isAction())
            subject = "Corrective Action: " +  event.getType().getDisplayName() + " due";
        else
            subject = "Event: " +  event.getType().getDisplayName() + " due";
        return subject;
    }

    private String getGoogleCalendarDates(Event event) {
        Date startDate = event.getDueDate();
        Date endDate;

        SimpleDateFormat dateFormat;
        //Check if its an all day event
        if (DateUtils.truncate(startDate, Calendar.DATE).equals(startDate)) {
            dateFormat = ALL_DAY_DATE_FORMAT;
