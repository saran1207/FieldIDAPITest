...
public class EventCompletenessChartSeries extends ChartSeries<LocalDate> {
    ...
    public EventCompletenessChartSeries(WorkflowState workflowState, List<EventCompletenessReportRecord> events) {
        super(workflowState, workflowState.getLabel(), new ArrayList());
        add(getEvents(events,workflowState));
    }

    private List<EventCompletenessReportRecord> getEvents(List<EventCompletenessReportRecord> events, final WorkflowState workflowState) {
