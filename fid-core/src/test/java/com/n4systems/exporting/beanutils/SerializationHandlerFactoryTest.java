...
public class SerializationHandlerFactoryTest {
	...
	public void test_get_sorted_handlers_skip_import_only_fields() throws InstantiationException {
		SerializationHandlerFactory factory = new SerializationHandlerFactory(true);
		...
		// will skip the "importOnlyText" field that would be positioned here but it was filtered out by handler.
		
		assertTrue(handlers[4] instanceof SimpleSerializationHandler);		
		assertEquals("date", handlers[4].getField().getName());
		
		assertTrue(handlers[5] instanceof MapSerializationHandler);		
		assertEquals("map", handlers[5].getField().getName());
