...
public class ApiAssetTypeAttachmentResource extends ApiResource<ApiAssetTypeAttachment, FileAttachment> {
	...
	public List<ApiModelHeader> query(@QueryParam("assetTypeIds") List<Long> assetTypeIds, @QueryParam("id") List<String> attachmentIds) {
		List<FileAttachment> attachments = (attachmentIds != null && attachmentIds.size() > 0) ? findAttachmentsByMobileIds(attachmentIds) : findAttachmentsByAssetTypes(assetTypeIds);
		List<ApiModelHeader> headers = convertAllEntitiesToApiModels(attachments, a -> new ApiModelHeader(a.getMobileId(), a.getModified()));
		return headers;
	}
