...
public class ApiProcedureDefinitionResource extends ApiResource<ApiProcedureDefinition, ProcedureDefinition> {
	private static Logger log = Logger.getLogger(ApiProcedureDefinitionResource.class);
	...
	public List<ApiModelHeader> queryAsset(@QueryParam("id") List<ApiKeyString> assetIds) {
		...
		QueryBuilder<ApiModelHeader> query = new QueryBuilder<>(ProcedureDefinition.class, securityContext.getUserSecurityFilter(true));
		query.setSelectArgument(new NewObjectSelect(ApiModelHeader.class, "mobileId", "modified"));
		query.addWhere(WhereClauseFactory.create(WhereParameter.Comparator.IN, "asset.mobileGUID", unwrapKeys(assetIds)));
		List<ApiModelHeader> results = persistenceService.findAll(query);
		return results;
	}
	...
	public List<ApiProcedureDefinition> findAll(@QueryParam("id") List<ApiKeyString> ids) {
		...
		query.addWhere(WhereClauseFactory.create(WhereParameter.Comparator.IN, "mobileId", unwrapKeys(ids)));
		List<ProcedureDefinition> results = persistenceService.findAll(query);
		List<ApiProcedureDefinition> apiModels = convertAllEntitiesToApiModels(results);
		return apiModels;
	}
	...
	protected List<ApiProcedureDefinition> convertAllProcedureDefinitionsToApiModels(List<ProcedureDefinition> procedureDefinitions) {
	...
	private ProcedureDefinition convertApiModelToEntity(ApiProcedureDefinition apiProcDef, ProcedureDefinition procDef, boolean isNew) throws ImageProcessingException, PersistenceException {
		...
		//Transfer the easy fields from the ApiProcedureDefinition object.
...
//        entityAnnotation.setImage(image);
	...
	public ProcedureDefinition convertToEntityIsolationPoints(ProcedureDefinition procDef,
															  ApiProcedureDefinition apiProcDef, boolean isNew)
	...
	private IsolationDeviceDescription convertDefinition(ApiDeviceDescription deviceDefinition) {
		...
		description.setAttributeValues(new ArrayList<>(apiAttributeValueConverter.convertAttributeValues(deviceDefinition.getAttributes(), null)));
