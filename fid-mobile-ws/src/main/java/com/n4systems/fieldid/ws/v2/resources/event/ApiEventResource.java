...
public class ApiEventResource extends ApiResource<ApiEvent, ThingEvent> {
	...
	public List<ApiModelHeader> queryById(@QueryParam("id") List<String> eventIds) {
	...
	@Produces(MediaType.APPLICATION_JSON)
	@Transactional(readOnly = true)
	public List<ApiModelHeader> queryCompleted(@QueryParam("assetId") List<String> assetIds) {
		if (assetIds.isEmpty()) return new ArrayList<>();

		List<ApiModelHeader> headers = eventService.getLastEventOfEachType(assetIds)
				.stream()
	...
	@Produces(MediaType.APPLICATION_JSON)
	@Transactional(readOnly = true)
	public List<ApiModelHeader> queryOpenCompleted(@QueryParam("assetId") List<String> assetIds, @DefaultValue("YEAR") @QueryParam("syncDuration") SyncDuration syncDuration) {
		if (assetIds.isEmpty()) return new ArrayList<>();
	...
	@Produces(MediaType.APPLICATION_JSON)
	@Transactional(readOnly = true)
	public List<ApiEvent> findAll(@QueryParam("id") List<String> eventIds) {
		if (eventIds.isEmpty()) return new ArrayList<>();

		QueryBuilder<ThingEvent> queryBuilder = createUserSecurityBuilder(ThingEvent.class);
		queryBuilder.addWhere(WhereClauseFactory.create(WhereParameter.Comparator.IN, "mobileGUID", eventIds));
