...
public class ApiAssetAttachmentResource extends ApiResource<ApiAssetAttachment, AssetAttachment> {
	...
	public List<ApiModelHeader> findAllForAsset(@QueryParam("id") List<String> assetIds)
	{
		QueryBuilder<ApiModelHeader> queryBuilder = new QueryBuilder<>(AssetAttachment.class, securityContext.getUserSecurityFilter());
		queryBuilder.addWhere(WhereClauseFactory.create(Comparator.IN, "asset_id", assetIds));
		queryBuilder.setSelectArgument(new NewObjectSelect(ApiModelHeader.class, "sid", "modified"));
