...
public class ApiAssetTypeAttachmentResource extends ApiResource<ApiAssetTypeAttachment, FileAttachment> {
	...
	@Produces(MediaType.APPLICATION_JSON)
	@Transactional(readOnly = true)
	public List<ApiModelHeader> queryAssetType(@QueryParam("assetTypeIds") List<ApiKeyLong> assetTypeIds) {
		if (assetTypeIds.isEmpty()) return new ArrayList<>();
		List<ApiModelHeader> headers = convertAllEntitiesToApiModels(findAttachmentsByAssetTypes(unwrapKeys(assetTypeIds)), a -> new ApiModelHeader(a.getMobileId(), a.getModified()));
		return headers;
	}
	...
	@Produces(MediaType.APPLICATION_JSON)
	@Transactional(readOnly = true)
	public List<ApiAssetTypeAttachment> findAll(@QueryParam("id") List<ApiKeyString> attachmentIds) {
		if (attachmentIds.isEmpty()) return new ArrayList<>();
