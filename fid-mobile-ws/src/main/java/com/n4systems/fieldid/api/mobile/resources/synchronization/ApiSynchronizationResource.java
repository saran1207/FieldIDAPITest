...
import com.n4systems.fieldid.api.mobile.resources.model.ListResponse;
import com.n4systems.fieldid.api.mobile.resources.procedure.ApiProcedureResource;
...
public class ApiSynchronizationResource extends FieldIdPersistenceService {
	...
	private List<ApiSynchronizationAsset> getAssignedOpenEventAssets(OfflineProfile profile) {
		...
		if (user.getGroups().isEmpty()) {
	        ...
	        group.addClause(WhereClauseFactory.create(Comparator.EQ, "assignee.id", user.getId(), WhereClause.ChainOp.OR));
	        group.addClause(WhereClauseFactory.create(Comparator.IN, "assignedGroup", user.getGroups(), WhereClause.ChainOp.OR));
