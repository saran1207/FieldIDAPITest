...
public class ApiPlaceEventTypeResource extends SetupDataResource<ApiEventType, PlaceEventType> {
	...
	protected ListResponse<ApiEventType> getApiPage(DateParam after, int page, int pageSize) {
		List<PlaceEventType> placeEventTypes = persistenceService.findAll(createFindAllBuilder(after));

		List<ApiEventType> apiModels;
		int startIndex = page * pageSize;

		if (startIndex > placeEventTypes.size()) {
			apiModels = new ArrayList<>();
		} else {
			int endIndex = startIndex + pageSize;
			if (endIndex > placeEventTypes.size()) {
				endIndex = placeEventTypes.size();
			}
