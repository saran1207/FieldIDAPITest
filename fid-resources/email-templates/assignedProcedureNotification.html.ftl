                            ...
                            <tr>
                                <td width="600" valign="top" class="no-image-adjustment">
                                    <p class="information-adjustment information-paragraph">
                                        <strong>LOTO Procedure:</strong> ${procedure.type.procedureCode} <br>
                                        <strong>Asset:</strong>&nbsp;
                                        <#if showLinks>
                                            <a target="_blank" href="${assetUrlMap.get(procedure.asset.id)}">${procedure.asset.type.name} / ${procedure.asset.identifier}</a>
                                        <#else>
                                        ${procedure.asset.type.name} / ${procedure.asset.identifier}
                                        </#if>
                                        <br>
                                        <strong>Owner:</strong> ${procedure.asset.owner.displayName}
                                        <br>
                                        <strong>Due:</strong> ${dueDateStringMap.get(procedure.id)}
                                        <br>
                                        <#if procedure.modifiedBy??>
                                            <strong>Assigned By:</strong> ${procedure.modifiedBy.fullName!}
                                            <br>
                                        </#if>
                                    </p>
                                </td>
