...
public class ArrowStyleIsolationPointImageGallery extends ArrowStyleEditableImageGallery<ProcedureDefinitionImage> {
    ...
    public ArrowStyleIsolationPointImageGallery(String id, ProcedureDefinition procedureDefinition, IModel<IsolationPoint> model) {
    ...
    @Override
    protected ImageAnnotation getImageAnnotation(Long id, Double x, Double y, Double x2, Double y2, String text, ImageAnnotationType type) {
        ImageAnnotation annotation = getIsolationPoint().getAnnotation();
        if (annotation==null && id==null) {   // create new one.
            annotation = new ImageAnnotation(x, y, x2, y2, text, type);
        ...
        annotation.setX(x);
        annotation.setY(y);
        annotation.setX_tail(x2);
        annotation.setY_tail(y2);
