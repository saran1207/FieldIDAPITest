...
public class ProcedurePickerPanel extends Panel {
    ...
    public ProcedurePickerPanel(String id, final IModel<Procedure> scheduleModel) {
        ...
        assigneeOrDateUpdated = scheduleModel.getObject().isAssigneeOrDateChanged();
        ...
        noActiveProcedureDefinitionMessage.add(new AjaxLink("copyExistingProcedureLink") {
            @Override
            public void onClick(AjaxRequestTarget target) {
                setResponsePage(new CopyProceduresList(scheduleModel.getObject().getAsset()));
            }
        });
    ...
    class ProcedureForm extends Form<Procedure> {
        ...
        public ProcedureForm(String id, IModel<Procedure> procedureScheduleModel) {
            ...
            add(new DropDownChoice<>("procedureDefSelect",
                    new PropertyModel<>(procedureScheduleModel, "type"),
                    choices,
                    new ListableChoiceRenderer<>())
                    .setNullValid(false)
                    .setRequired(true)
                    .add(new JChosenBehavior()));


            add(dateTimePicker = new DateTimePicker("datePicker", new PropertyModel<>(procedureScheduleModel, "dueDate"), true) {
                @Override
            ...
            AjaxSubmitLink addScheduleButton =  new AjaxSubmitLink("addScheduleButton") {
                ...
                protected void onSubmit(AjaxRequestTarget target, Form<?> form) {
                    if(assigneeOrDateUpdated) {
                        scheduleModel.getObject().setAssigneeOrDateChanged();
                    }
            ...
            add(new CheckBox("sendEmailOnUpdate", new PropertyModel<>(scheduleModel, "sendEmailOnUpdate")));
