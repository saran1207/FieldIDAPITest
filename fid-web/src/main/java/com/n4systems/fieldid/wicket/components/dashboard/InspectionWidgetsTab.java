 ...
 */
public class InspectionWidgetsTab extends Panel {
    ...
    public InspectionWidgetsTab(String id, IModel<DashboardLayout> currentLayoutModel) {
        super(id);
    ...
    private LoadableDetachableModel<List<WidgetType>> createWidgetListModel() {
        return new LoadableDetachableModel<List<WidgetType>>() {
            ...
            protected List<WidgetType> load() {
                ...
                List<WidgetType> inspectionList = new ArrayList<WidgetType>();

                for(WidgetType widgetType:widgetTypeList) {
                    if(widgetType.isInspection()){
                        inspectionList.add(widgetType);
                    }
                }

                if(!FieldIDSession.get().getPrimaryOrg().hasExtendedFeature(ExtendedFeature.Projects)) {
                    inspectionList.remove(WidgetType.JOBS_ASSIGNED);
                }
