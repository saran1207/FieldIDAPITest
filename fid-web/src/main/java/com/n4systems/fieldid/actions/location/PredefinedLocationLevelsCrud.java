...
public class PredefinedLocationLevelsCrud extends AbstractCrud {
	...
	private Logger logger = Logger.getLogger(PredefinedLocationLevelsCrud.class);
	private LevelNameWebModel levelName;
	private int nodeLevel;
	private long nodeId;

	public PredefinedLocationLevelsCrud(PersistenceManager persistenceManager) {
	...
	public String doCreate() {
		...
		if(locationLevels.getLevels().size() < levelName.getIndex()) {
			for (int i = locationLevels.getLevels().size(); i < levelName.getIndex(); i++) {
				locationLevels.getLevels().add(i, new LevelName());
			}
		}
