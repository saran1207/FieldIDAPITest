			...
			<#if (linkedAsset.type.cautions?exists && linkedAsset.type.cautions?length gt 0) ||
				...
				<div class="viewSection smallViewSection" >
					...
					<#if linkedAsset.type.imageName?exists >
						<p>
							<img src="<@s.url action="downloadProductTypeImageSafetyNetwork" namespace="/file" uniqueID="${linkedAsset.type.uniqueID}" networkId="${linkedAsset.networkId}"/>" alt="<@s.text name="label.productimage"/>" width="300"/>
						</p>
					</#if>
