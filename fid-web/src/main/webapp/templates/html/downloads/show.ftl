			...
			<tr id="dl_${download.id}" class="${style}">
				...
				<td class="normal" > 
					...
					<#if download.state.ready>
						...
						|
						<a href='<@s.url action="emailDownload" namespace="/aHtml/iframe" fileId="${download.id}"/>' class='lightview' rel='iframe' title=" :: :: autosize: true, scrolling:true, width: 830, height: 500" ><@s.text name="label.share" /></a>
						|
