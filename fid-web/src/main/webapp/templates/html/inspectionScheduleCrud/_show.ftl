...
<#if sessionUser.hasAccess( "createinspection" ) && !inVendorContext>
	<span>
		...
		<a id="inspectNow_${inspectionSchedule.id}" <#if inspectionSchedule.status.getName() == "IN_PROGRESS">style="display:none"</#if> href='<@s.url action="selectEventAdd" namespace="/" assetId="${assetId}" type="${inspectionSchedule.inspectionType.id}" scheduleId="${inspectionSchedule.id}" />'><@s.text name="label.inspect"/></a>
