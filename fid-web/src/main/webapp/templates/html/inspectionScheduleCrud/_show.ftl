...
<#if sessionUser.hasAccess( "createinspection" ) && !inVendorContext>
	<span>
		...
		<a id="inspectNow_${inspectionSchedule.id}" <#if inspectionSchedule.status.getName() == "IN_PROGRESS">style="display:none"</#if> href='<@s.url action="selectInspectionAdd" namespace="/" assetId="${assetId}" type="${inspectionSchedule.inspectionType.id}" scheduleId="${inspectionSchedule.id}" />'><@s.text name="label.inspect"/></a>
		<a id="stopProgress_${inspectionSchedule.id}" <#if inspectionSchedule.status.getName() == "SCHEDULED">style="display:none"</#if> href='javascript:void(0);' onclick="getResponse('<@s.url action="inspectionScheduleStopProgress" namespace="/ajax" assetId="${assetId}" uniqueID="${inspectionSchedule.id}" />'); return false"><@s.text name="label.stopprogress"/></a>
	</span>
</#if>	
