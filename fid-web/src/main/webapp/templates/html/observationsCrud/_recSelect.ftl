<div class="observationBox" id="rec_${criteria.id}" style="display: none;">
	...
	<#list criteria.recommendations as recommendation >
	  	...
	  	<#assign notSelected=!(criteriaResults[currentCriteriaIndex].recommendations[recommendation_index])?exists />
		<#if notSelected>
			<#assign stateValue="NOTSELECTED" />
		<#else>
			<#assign stateValue=criteriaResults[currentCriteriaIndex].recommendations[recommendation_index].stateString />
		</#if>
		...
		<#if stateValue != "COMMENT">
			...
			<p id="rec_${criteria.id}_${recommendation_index}" class="observationText ${selectClass}" onclick="cycleRecommendation(${criteria.id}, ${recommendation_index})" >
				...
				<#if (criteriaResults[currentCriteriaIndex].recommendations[recommendation_index].id)?exists >
					<@s.hidden id="rec_${criteria.id}_${recommendation_index}_id" name="criteriaResults[${currentCriteriaIndex}].recommendations[${recommendation_index}].iD" value="${criteriaResults[currentCriteriaIndex].recommendations[recommendation_index].id}" />
				</#if>
	...
	<p class="observationComment" >
		...
		<#if (criteriaResults[currentCriteriaIndex].recommendations[criteria.recommendations.size()].id)?exists >
			<@s.hidden id="rec_${criteria.id}_${criteria.recommendations.size()}_id" name="criteriaResults[${currentCriteriaIndex}].recommendations[${criteria.recommendations.size()}].iD" value="${criteriaResults[currentCriteriaIndex].recommendations[criteria.recommendations.size()].id}" />
		</#if>
		<label><@s.text name="label.comments"/></label>
		<span><@s.textarea  id="rec_${criteria.id}_${criteria.recommendations.size()}_text" name="criteriaResults[${currentCriteriaIndex}].recommendations[${criteria.recommendations.size()}].text" cols="53" rows="3" onfocus="captureRecCommentState(this.value);" onchange="checkRecComment(${criteria.id}, this.value);" value="${(criteriaResults[currentCriteriaIndex].recommendations[criteria.recommendations.size()].text)!}"/></span>
