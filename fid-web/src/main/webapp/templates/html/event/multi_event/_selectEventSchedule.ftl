...
<@s.form action="retrieveEventDetails" namespace="/multiEvent/ajax" id="retrieveEventDetails" cssClass="fullForm fluidSets">
	...
	<#list assets as asset>
		...
		<div class="infoSet">
			...
			<@s.select id="${asset.id!}_eventSchedules" name="eventScheduleIdToComplete" headerKey="-1" headerValue="${action.getText('label.notscheduled')}" list=eventSchedules listKey="id" listValue="nextDate"/>
		</div>
	</#list>
