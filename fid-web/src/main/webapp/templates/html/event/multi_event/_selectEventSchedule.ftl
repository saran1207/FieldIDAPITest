	...
	<div class="schedulesBorder">
		<#list assets as asset>
			...
			<div class="infoSet scheduleContainer">
				...
				<@s.select id="eventScheduleSelectBox_${asset_index}" cssClass="eventSchedules" headerKey="0" headerValue="${action.getText('label.notscheduled')}" list=eventSchedules listKey="id" listValue="dueDate" value=multiEventScheduleListHelper.getSuggestedEventScheduleIdForAsset(asset) onchange="storeScheduleId(${asset_index},this)"/>
			</div>
		</#list>
	</div>
