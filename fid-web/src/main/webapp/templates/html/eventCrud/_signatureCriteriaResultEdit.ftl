<div id="signatureCriteria${criteriaId}" class="criteriaEditContainer">
    <#if signatureFileId?exists && signatureFileId != "">
        ...
        <a class="clearSignatureLink" onclick="clearSignature(${criteriaId}, ${criteriaCount})"><@s.text name="label.clear_signature"/></a>
    ...
    <#else>
        ...
        <a href="/fieldid/aHtml/sign.action" criteriaid="${criteriaId}" criteriacount="${criteriaCount}" class="lightview signLink" rel="iframe" title="Signature :: :: scrolling:false, width: 790, height: 300"><@s.text name="label.sign"/></a>
    </#if>
</div>