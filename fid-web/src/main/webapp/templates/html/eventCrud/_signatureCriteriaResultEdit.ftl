<div id="signatureCriteria${criteriaId}" class="criteriaEditContainer">
    <#if signatureFileId?exists>
        <img src="<@s.url action="downloadTemporarySignature" namespace="/file" signatureFileId="${signatureFileId}"/>" width="150" height="39"/><br/>
    ...
    <#elseif criteriaResult?exists && criteriaResult.signed>
        <img src="<@s.url action="downloadSignature" namespace="/file" eventId="${event.id}" criteriaId="${criteriaResult.criteriaId}"/>" width="150" height="39"/><br/>
        <input type="hidden" name="criteriaResults[${criteriaCount}].signed" value="false"/>
        <a onclick="clearSignature(${criteriaId}, ${criteriaCount})">Clear signature</a>
    <#else>
        <a href="/fieldid/aHtml/sign.action" criteriaid="${criteriaId}" criteriacount="${criteriaCount}" class="lightview" rel="iframe" title="Signature :: :: scrolling:false, width: 800, height: 250">Sign</a>
    </#if>
</div>