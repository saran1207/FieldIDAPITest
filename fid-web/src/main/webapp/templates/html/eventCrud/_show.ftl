		...
		<p>
			...
			<span>
                <#if inside_iframe?exists>
                    <a href="#" onclick="parent.window.location='<@s.url value="/w/assetSummary?uniqueID=${asset.id}" />'">${asset.identifier?html}</a>
                <#else>
                    <a href="<@s.url namespace="/" value="w/assetSummary?uniqueID=${asset.id}"/>">${asset.identifier?html}</a>
                </#if>
			</span>
		</p>
		<p>
